...
const [dropdownVisible2, setDropdownVisible2] = useState(false);
  ...
  const ScrollToTop = () => {
        ...
        // Scroll to the top when the user scrolls
        window.scrollTo(0, 0);
...
function setmove5(){
  ...
  const handleImageClick = (product) => {
    ...
    navigatee(`/loan/loanProductDetails?/img=${product.img}&name=${product.name}&id=${product.id}&price=${product.price}&type=${product.type}&/lineImage1=${product.lineImage1}&/lineImage2=${product.lineImage2}&/lineImage3=${product.lineImage3}&/lineImage4=${product.lineImage4}&/lineImage5=${product.lineImage5}&/lineImage6=${product.lineImage6}&size=${product.size}&clothtype=${product.clothtype}&pricelogo=${product.pricelogo}&priceunit=${product.priceunit}&pricee=${product.pricee}`);
  };
  const { cart,instock4,instocker4,setCart,addToCart, decreaseFromcart,removeFromcart,addToCart2, decreaseFromcart2 ,quantitySelector,SetQuantity,SetQuantity2,Find,Finder,Filter,FindShow, setFindShow,Setfilter,handleSearchInputChange,ChangePrice,unit,unitChanger} = useContext(CartContext); // Access cart and addToCart from CartContext
...
let filterProductsByPrice= function () {
  let filteredProducts = loan.filter((product) => {
...
function showOnFront(id){
  instock4.filter(function(o){return o.id===id })
}
  ...
  return (
    ...
    <div className="cover-all">
      <div className="upper-section">
        ...
        <div className={`navbar nav-bod${isSticky ? "sticky-navbar" : "" }`} style={{background:'white'}}>
...
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/loan/unstiched' >UNSTICHED</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/loan/chantelle' >CHANTELLE</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/loan' >LOAN</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/loan/shaal' >SHAAL</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
...
<Link to='/loan/login'><img src="/assests/pngwing.com (35).png" alt=""  width={'20px'} className='hidden'/></Link>
 ...
 <div key={product.id} style={{margin: '19px 6px'}} className='contains-all'>
  ...
  <div className="quantity-checker">
    <div className="add-minus-2" style={{margin:' 32px 0'}}>
      <button className='add-button-3' onClick={function(){addToCart(product,selectedSizes[product.id],TypeSelection[product.id]),toggleSidebar2(),toggleOnFront()}}>ADD TO CART</button>
...
<div className="search-showing-area">
      ...
      <div className='center-div'>LOAN</div>
      <div className="allll-2">
      <div className='center-div-2' > <div className='products-bar'> <div className='length-1'>{loan.length}</div> PRODUCTS</div> <div className='new-filter' onClick={toggleSidebar5}><span>Filter</span></div></div>
...
<div className="overlay"></div>
          <div className="on-front-element" >
            ...
            <div className={`pop-up-5 ${isPopupVisible ? 'show-trans' : ''}`}>
              <div className="remove-button-2">
