...
// Add a scroll event listener to detect when the navbar becomes sticky
  ...
  const { cart,instock,instocker,setCart,addToCart, decreaseFromcart,removeFromcart,addToCart2, decreaseFromcart2 ,quantitySelector,SetQuantity,SetQuantity2,instock2,instocker2,Find,Finder,Filter,FindShow, setFindShow,Setfilter,handleSearchInputChange,ChangePrice,unit,unitChanger} = useContext(CartContext); // Access cart and addToCart from CartContext
...
let filterProductsByPrice= function () {
  let filteredProducts = shaal.filter((product) => {
...
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/shaal/unstiched' >UNSTICHED</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/shaal/chantelle' >CHANTELLE</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/shaal/loan' >LOAN</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/shaal' >SHAWLS</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
...
<Link to='/shaal/login'><img src="/assests/pngwing.com (35).png" alt=""  width={'20px'} /></Link>
  </div>
    </div>
  </div>
...
<Link to='/shaal/login'><img src="/assests/pngwing.com (35).png" alt=""  width={'20px'} className='hidden'/></Link>
...
<img src="/assests/pngwing.com (37).png" alt=""  width={'20px'} />
  ...
  <p  className="p-1" style={{textAlign:'center', marginBottom:'18px'}}>{item.clothtype}</p>
  ...
  <div className="checkout-button">
    ...
    <p className="p-5-5">
      <div className='one-sided'>
        <span className='checkout-name'>CHECKOUT <span style={{marginLeft: '11px'}} className='line-straight'>|</span></span>
        <div></div>
      </div>
 ...
 <div key={product.id} style={{margin: '19px 6px'}} className='contains-all'>
  ...
  <div className="quantity-checker">
    <div className="add-minus-2" style={{margin:' 32px 0'}}>
      ...
      <div className='center-div'>SHAWLS</div>
      <div className="allll-2">
      <div className='center-div-2' > <div className='products-bar'> <div className='length-1'>{shaal.length}</div> PRODUCTS</div> <div className='new-filter' onClick={toggleSidebar5}><span>Filter</span></div></div>
