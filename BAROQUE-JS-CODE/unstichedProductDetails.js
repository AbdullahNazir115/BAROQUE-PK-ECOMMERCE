...
import {UnstichedProducts} from './CartProvider.js';
import './baroque.css';
import { Link } from "react-router-dom";
import Slider from "react-slick";// Import slick-carousel styles
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css"; 
import CartContext from './CartContext.js'; // Import 
...
function Productdetails() {
  ...
  const { cart, addToCart,decreaseFromcart ,removeFromcart,addToCart2,decreaseFromcart2,quantitySelector,SetQuantity,SetQuantity2,Find,Finder,Filter,FindShow, setFindShow,Setfilter,handleSearchInputChange,ChangePrice,unit,unitChanger,instock2} = useContext(CartContext); // Access cart and addToCart from CartContext
...
const selectedProduct = UnstichedProducts.find((product) => product.id === selectedProductId);
...
selectedProductLineImage2,
  ...
  selectedProductLineImage4,
  selectedProductLineImage5,
  selectedProductLineImage6,
];
let ju
console.log('length '+carouselImages[0])
// const initialImageIndex = tell ? 5 : 2;
const clickedImageIndex = carouselImages.findIndex((image) => image === selectedProductImg);
const initialImageIndex = 0; // Set this to the index of the 2nd image
...
// Calculate the total number of images for the product
...
function nextImage2() {
  setCurrentImageIndex2((prevIndex) => {
    const newIndex = (prevIndex + 1) % totalImages2;
    return newIndex === maxImages2 ? 0 : newIndex;
  });
}
...
function prevImage2() {
  setCurrentImageIndex2((prevIndex) => {
    const newIndex = prevIndex === 0 ? totalImages2 - 1 : prevIndex - 1;
    return newIndex === maxImages2 - 1 ? 0 : newIndex;
  });
}
...
// const [currencyPrice, setCurrencyPrice] = useState(product.price);
// function Changecurrency  (unit, newCurrency, newLogo) {
//   setSelectedCurrency(newCurrency);
//   setCurrencyLogo(newLogo);
// setCurrencyPrice( (product.price /unit).toFixed(2))
//   // You may also want to update the price in your cart items here
//   // or trigger a re-render of the cart details
// };
...
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/unstichedProductDetails/unstiched' >UNSTICHED</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/unstichedProductDetails/chantelle' >CHANTELLE</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/unstichedProductDetails/loan' >LOAN</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/unstichedProductDetails/shaal' >SHAAL</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
...
<Link to='/unstichedProductDetails/login'><img src="/assests/pngwing.com (35).png" alt=""  width={'20px'} /></Link>
  </div>
    </div>
...
<Link to='/unstichedProductDetails/login'><img src="/assests/pngwing.com (35).png" alt=""  width={'20px'} className='hidden'/></Link>
...
<div className="right-show">
 ...
 <div key={product.id} style={{margin: '19px 6px'}} className='contains-all'>
  ...
  <div className="quantity-checker">
    ...
    <button className='add-button-3' onClick={function(){ addToCart(product, selectedSizes[selectedProductId],TypeSelection[selectedProductId]),toggleSidebar2(),toggleOnFront()}}>ADD TO CART</button>
    </div>
     </div>
...
<p style={{fontSize:'17px' ,}}>SIZE :<p className="p-4"></p>{selectedSizes[selectedProductId]}</p>
<div className="sizes-3">
  <div className="left-button">
<button   onClick={() => handleSizeSelection(selectedProductId, 'XS')}>XS</button>
<button   onClick={() => handleSizeSelection(selectedProductId, 'L')}>L</button>
<button   onClick={() => handleSizeSelection(selectedProductId, 'M')}>M</button>
<button   onClick={() => handleSizeSelection(selectedProductId, 'S')}>S</button>
</div>
<div className="custom-button-div">
<button onClick={() => handleSizeSelection(selectedProductId, 'CUSTOM')}  className='custom-button-2'>CUSTOM</button>
</div>
</div>
...
<div className="quantity-checker">
  <div className="add-minus-2" >
    <button className='add-button-3' onClick={function(){ addToCart(product, selectedSizes[selectedProductId],TypeSelection[selectedProductId]),toggleSidebar2()}}>ADD TO CART</button>
    <button className='add-button'>BUY NOW</button>
    <div className="buy-now">
