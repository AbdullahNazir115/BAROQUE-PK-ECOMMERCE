...
const [isActive6, setIsActive6] = useState(false);
  ...
  function handleScroll10() {
    if (window.pageYOffset >650 /* Add the scroll threshold here */ ) {
      setIsSticky9(true);
    } else {
      setIsSticky9(false);
    }
...
<link href="https://fonts.googleapis.com/css2?family=Quicksand&display=swap" rel="stylesheet" />
    ...
    <header>
      ...
      <div className={`navbar ${isSticky ? "sticky-navbar" : "" }`} style={{  borderBottom: '1px solid #0000004f'}}>
...
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/login/unstiched' >UNSTICHED</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/login/chantelle' >CHANTELLE</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/login/loan' >LOAN</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
<li className={`${sidebarVisible ? 'animation-6' : 'animation-5'}`}><Link to='/login/shaal' >SHAAL</Link></li>
<hr width={'99%'} style={{color:'rgba(0, 0, 0, 0.18)'}}/>
...
<Link to='/login'><img src="/assests/pngwing.com (35).png" alt=""  width={'20px'} /></Link>
  </div>
    </div>
  </div>
...
<div className="together-7">
      <button className="dropbtn"  onClick={setmove5}><div className="triangle-down"><p>{element1Content}</p></div></button>
      <div className={`dropdown-content ${movedd ? 'animation-8' : 'animation-7'}`} style={{position:'relative'}} >
  <a  className={`${movedd?'animation-10' :''}`} onClick={function(){
    setmove5()
    ChangePrice(1,'PKR','') 
  e1()
    }}>PKR</a>
...
<div className="login-form">
    <div className="middle-3">
        <h1 className='login-2'>LOGIN</h1>
        <p className='f-family sizes-5'>Enter your email and password to login:</p>

    <input className='email f-family' type="email" name="" id="" placeholder='E-mail'/>
    <input className='email f-family' type="password" name="" id="" placeholder='Password' />
    <button className='login'>LOGIN</button>
    <p className='f-family sizes-5' style={{color:' #0000006e'}}> Don't have an account?<span style={{cursor:'pointer'}}> Sign up </span></p>
    </div>
</div>
